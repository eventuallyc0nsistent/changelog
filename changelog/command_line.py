...
def write_logs(new_version, logs):
        ...
        if issues:
            for issue in issues:
                if ('Merge branch' in title) or ('Merge pull' in title):
                    continue
                print '- {0} ([{1}](https://company.com/browse/{1}))'.format(title, issue)
...
def main(from_, to, new_version, issue_id_pattern):
    ...
    write_logs(new_version, logs)

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Publish CHANGELOG from git commits')
    ...
    parser.add_argument('-i', '--issue', type=str, 
                        help='Enter JIRA issue ID pattern. Default set to EKDATA-\d+')
    ...
    main(from_, to_, new_version, issue_id_pattern)
